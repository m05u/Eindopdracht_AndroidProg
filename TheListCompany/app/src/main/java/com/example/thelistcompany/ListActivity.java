...
public class ListActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        startDateEntry.setText(getTodaysDate());
        endDateEntry.setText(getTodaysDate());
        startDialog = initDatePicker(startDateEntry);
        endDialog =  initDatePicker(endDateEntry);
    ...
    private DatePickerDialog initDatePicker(Button button) {
        DatePickerDialog.OnDateSetListener dateSetListener = new DatePickerDialog.OnDateSetListener() {
            @Override
            public void onDateSet(DatePicker datePicker, int year, int month, int day) {
                month = month + 1;
                String date = makeDateString(day, month, year);
//                startDateEntry.setText(date);
                button.setText(date);
            }
        };
